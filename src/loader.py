...
class TsvLoader:
    ...
    def update_tsvs(self):
        get_dcc_df().to_csv(
            os.path.join(self.tsv_dir, "dcc.tsv"), sep="\t", index=False
        )
        get_id_namespace().to_csv(
            os.path.join(self.tsv_dir, "id_namespace.tsv"), sep="\t", index=False
        )
...
def add_project_title_row(project_file_path):
    ...
    project_df = pd.concat(
        [pd.DataFrame(project_title_row, index=[0]), project_df]
